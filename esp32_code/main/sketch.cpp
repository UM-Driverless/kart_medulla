...
void processGamepad(ControllerPtr ctl) {
    ...
    // ============================================================================
    // Read AS5600 Steering Sensor (optional for testing)
    // ============================================================================
    float currentAngle = 0.0f;  // Default to center position
    bool sensorAvailable = steeringSensor.isConnected();

    if (sensorAvailable) {
        currentAngle = steeringSensor.readAngleDegrees();  // -180 to 180 degrees
    } else {
        // Sensor not connected - use dummy value for testing
        // This allows controller testing without sensor hardware
        currentAngle = 0.0f;  // Assume centered
    }
    ...
    if (abs(axisX) > deadzone) {
        ...
        // axisX: -511 to 512 -> targetAngle: -MAX_STEERING_ANGLE to +MAX_STEERING_ANGLE
...
void setup() {
    ...
    // ============================================================================
    // Initialize AS5600 Steering Sensor (Optional for Testing)
    // ============================================================================
    ...
    if (!steeringSensor.begin(PIN_I2C_SDA, PIN_I2C_SCL)) {
        ...
        Console.println("System will work without sensor for controller testing");
        Console.println("(Check I2C connections: SDA=21, SCL=22 for sensor hardware)");
    } else {
