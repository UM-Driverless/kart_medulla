...
/* ---------- SPI → MCP4922 external DAC (unused on this branch) ---------- */
/* The board's analog outputs come from an MCP4922 (U13) over SPI2, because the
 * S3 has no DAC of its own. That path produces nothing today: the SPI write was
 * never implemented. Rather than finish it, throttle is generated by filtered
 * PWM (see below). These stay defined because KM_GPIO_Init() still brings the
 * bus up, and the `spi-fix` branch carries a working implementation for
 * whoever wants to compare the two. */
...
/* ---------- Throttle: LEDC PWM + external RC low-pass ---------- */
/* GPIO 38 is unconnected on the PCB — it reaches an empty socket contact
 * (U24 pad 13, silkscreen `38` on the dev board's right edge) and no trace. The
 * analog throttle command is made by running a 12-bit LEDC carrier here and
 * averaging it with an RC network wired by hand to the MCP4922's output net
 * (`CMD_ACC_ESP32__0_5V`, U13 pin 14 / U14 pin 8), so the MAX4660 mux and its
 * fail-safe pedal pass-through stay in the path.
 *
 * A GPIO swings 0-3.3 V while that net is scaled for 0-5 V, so without a gain
 * stage full duty commands ~66% throttle. The wiring, the filter values and the
 * op-amp option are written up in history.md (2026-07-31).
 *
 * 19531 Hz is 80 MHz / 4096 — the fastest carrier that still leaves 12 bits of
 * duty resolution, which matters because ripple falls as the carrier rises. */
#define PIN_CMD_ACC             GPIO_NUM_38
#define THROTTLE_PWM_FREQ_HZ    19531
#define THROTTLE_PWM_TIMER      LEDC_TIMER_2
#define THROTTLE_PWM_CHANNEL    LEDC_CHANNEL_2

/* Brake still has no output on the S3. It is not a GPIO, and unlike throttle it
 * has no hand-wired substitute yet, so this stays a stand-in value: distinct
 * from any real pin, above the S3's GPIO range (0-48) so it cannot alias one,
 * and below 256 so km_act's uint8_t does not truncate it. It used to be
 * GPIO_NUM_NC (-1), which that uint8_t turned into 255 — and since throttle was
 * -1 too, the first `if` in KM_GPIO_WriteDAC() matched both channels. */
#define PIN_CMD_BRAKE           ((gpio_num_t)201)

#define PIN_SELECT_THROTTLE     GPIO_NUM_15  // MAX4660 mux; drive HIGH = throttle via PWM. GAP: not driven
